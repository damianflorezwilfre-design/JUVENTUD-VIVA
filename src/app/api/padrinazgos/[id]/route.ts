    ...
    if (session.role !== 'SUPER_ADMIN') {
      await prisma.editRequest.create({
        data: {
          ...
          recordId: id,
          proposedData: JSON.stringify({ title, description, goalAmount, currentAmount, imageUrl, isActive })
        }
      });
      return NextResponse.json({ success: true, message: 'Solicitud enviada', isRequest: true });
    }

    const actualizado = await prisma.sponsorship.update({
      where: { id },
      data: {
        title,
        description,
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (session.role !== 'SUPER_ADMIN') {
      await prisma.editRequest.create({
        data: { userId: session.id, action: 'DELETE', modelName: 'Sponsorship', recordId: id, proposedData: null }
      });
      return NextResponse.json({ success: true, message: 'Solicitud enviada', isRequest: true });
    }
